<?php
	...
	*/
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'Ads' => "`Ads`.`id` as 'id', `Ads`.`CreatedOn` as 'CreatedOn', `Ads`.`NickName` as 'NickName', `Ads`.`Sex` as 'Sex', `Ads`.`Age` as 'Age', `Ads`.`Status` as 'Status', `Ads`.`InterestedIn` as 'InterestedIn', `Ads`.`Pic1` as 'Pic1', `Ads`.`Pic2` as 'Pic2', `Ads`.`Pic3` as 'Pic3', `Ads`.`MoreInfo` as 'MoreInfo', `Ads`.`Contact` as 'Contact'"
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('katedate_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('katedate_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['katedate_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['katedate_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> katedate</a>
			</div>
